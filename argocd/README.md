...
The **hub** runs on AKS (free control plane; nodes paid by the $200/30-day
credit, so it stays up all build week for free). EKS is the **spoke**,
registered as an ArgoCD cluster secret and torn down daily to save money -
it re-joins the fleet in minutes when recreated.
...
kubectl apply -f argocd/monitoring-appset.yaml   # after creating grafana-cloud secrets
argocd app list   # watch both apps go Healthy/Synced
```
